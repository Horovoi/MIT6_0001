...
def is_valid_word(word, hand, word_list):
    ...
    # define a helper function to check if a word is a subset of a hand
    def is_subset(word, hand):
        """
        ...
        if word_freq.keys() <= hand.keys():
            for letter in word_freq.keys():
                if word_freq[letter] > hand[letter]:
                    return False
            return True
        else:
            return False
    ...
    if '*' in word:
        for vowel in VOWELS:
            if word.replace("*", vowel) in word_list and is_subset(word, hand):
                return True
        return False
    else:
...
def substitute_hand(hand, letter):
    ...
    n = hand[letter]
    new_hand = hand.copy()
    substitution = set(VOWELS + CONSONANTS) - set(hand.keys())
    new_letter = random.choice(''.join(substitution))

    del new_hand[letter]
    new_hand[new_letter] = new_hand.get(new_letter, 0) + n
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while flag:
        ...
        if count_substitute == 1:
            while substitute_test:
        ...
        score_list.append(play_hand(hand, word_list))
        print("----------")
